#!/usr/bin/env python3
"""
RunPod Serverless Handler cho Face Enhancement Service - FIXED VERSION
Fixed lỗi tuple index out of range và các vấn đề khác
"""
...
def get_file_type(file_path: str) -> str:
    """🔧 FIXED: Determine if file is video, image or unknown - Fixed tuple index error"""
    try:
        # 🔧 FIX: Use correct index [1] instead of [15]
        ...
        if ext in video_extensions:
            ...
            if os.path.exists(file_path):
                try:
                    # Simple content detection based on file size and header
        ...
        logger.warning(f"⚠️ Error determining file type for {file_path}: {e}")
        return 'unknown'
...
def handler(job: Dict[str, Any]) -> Dict[str, Any]:
    """
    🔧 FIXED: Main RunPod handler for Face Enhancement Service - All bugs fixed
    Fixed tuple index out of range error and enhanced error handling
    """
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix=temp_prefix) as temp_dir:
            ...
            if not input_filename or '.' not in input_filename:
                ...
                input_filename = f"input_{timestamp}.jpg"  # Default to jpg for images
            
            # Remove any problematic characters
            input_filename = "".join(c for c in input_filename if c.isalnum() or c in '._-')
            input_path = os.path.join(temp_dir, input_filename)

            # 🔧 FIXED: Use the fixed get_file_type function
            ...
            if file_type == 'video':
                ...
                # Unknown type - assume image
                output_filename = f"{name}_{enhancer}_b{int(blend*100)}_enhanced.jpg"
            ...
            return {
                ...
                "fix_version": "2.1_TUPLE_INDEX_FIXED"
            }
...
if __name__ == "__main__":
    logger.info("🚀 Starting Face Enhancement Serverless Worker (FIXED v2.1)...")
    ...
    try:
        ...
        logger.info("🎨 Face Enhancement Service Ready for Production (FIXED VERSION)!")
        ...
        logger.info(f"🔧 BUG FIXES: Tuple index error fixed, enhanced error handling")
